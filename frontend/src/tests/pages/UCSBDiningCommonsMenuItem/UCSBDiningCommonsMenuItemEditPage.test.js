...
describe("UCSBDiningCommonsMenuItemEditPage tests", () => {
    ...
    describe("tests where backend is working normally", () => {
        ...
        test("Is populated with the data provided", async () => {
            ...
            expect(nameField).toHaveValue("Bread");
            ...
            fireEvent.change(nameField, { target: { value: 'Soft Serve Icecream' } });
            fireEvent.change(diningCommonsCodeField, { target: { value: 'DLG' } });
            fireEvent.change(stationField, { target: { value: 'Desert' } });
            fireEvent.click(submitButton);

            await waitFor(() => expect(mockToast).toBeCalled());
            expect(mockToast).toBeCalledWith("UCSBDiningCommonsMenuItem Updated - id: 17 name: Soft Serve Icecream");
